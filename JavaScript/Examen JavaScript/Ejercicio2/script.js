...
function creaUsuario(){
    ...
    const datos= new FormData();
    datos.append(nick.value,JSON.stringify(usuario))
    const peticion= new XMLHttpRequest();
    peticion.open('POST','http://localhost:3000/users');//no he podido usarlo pero el metodo de envio seria algo asi
    peticion.send(datos);
    peticion.addEventListener('load', function(){
        if(peticion.status==200){
            res=peticion.responseText;
        }
        else{alert("Ha habido un error al enviar los datos")}
    })
...
function consulta(e){
    e.preventDefault();
    fetch('http://localhost:3000/users?nombre='+nombre.value)
    .then(response =>{
        if(response.ok){
